      ...
      if (nota.etiquetas != null) {
        if (nota.etiquetas!.isNotEmpty) {
          for (var tag in tagsList) {
            selectedTags.add(
                nota.etiquetas!.firstWhere((element) => element.id == tag.id));
          }
        }
      }
    }
  ...
  Widget htmlEditor() {
    return HtmlEditor(
      ...
      htmlEditorOptions: HtmlEditorOptions(
          initialText: nota.contenido, hint: 'recuperando contenido...'),
      htmlToolbarOptions: HtmlToolbarOptions(
          ...
          customToolbarButtons: [
            ElevatedButton(
              onPressed: () {
                showBottomSheet(
                    context: context, builder: (context) => menuOpciones());
              },
              child: const Icon(Icons.add),
              style: ElevatedButton.styleFrom(
                  backgroundColor: Color.fromARGB(255, 30, 103, 240)),
            )
          ],
          mediaUploadInterceptor: fileInterceptor), //
      otherOptions: OtherOptions(height: 550, decoration: BoxDecoration()),
    );
  }

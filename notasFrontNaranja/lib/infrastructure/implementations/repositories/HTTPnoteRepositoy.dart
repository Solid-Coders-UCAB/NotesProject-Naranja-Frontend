...
class httpNoteRepository extends HTTPrepository implements noteRepository {
  ...
  Future<Either<MyError, String>> createNota(Nota note) async {
    ...
    try {
      final Response response = await post(
          Uri.parse('https://$domain/nota/create'),
          body: body,
          headers: {
  ...
  Future<Either<MyError, List<Nota>>> getALLnotes(String userId) async {
    ...
    List<tarea> tareas = [];
    Response response;
    var body = jsonEncode({'idUsuario': userId});
    ...
    if (response.statusCode == 200) {
      ...
      for (var jsonNote in jsonData) {
        ...
        if (jsonAssignedGeolocalitation) {
          nota.latitud = double.parse(
              jsonNote['geolocalizacion']['value']['latitud'].toString());
          nota.longitud = double.parse(
              jsonNote['geolocalizacion']['value']['longitud'].toString());
        } else {
          nota.latitud = null;
          nota.longitud = null;
        }
  ...
  Future<Either<MyError, String>> updateNota(Nota note) async {
    ...
    try {
      final Response response = await put(
          Uri.parse('https://$domain/nota/modificate'),
          body: body,
          headers: {
            "Accept": "application/json",
            "content-type": "application/json"
  ...
  @override
  Future<Either<MyError, List<Nota>>> getAllEliminatedNotes(
      String userId) async {
    List<Nota> notas = [];
    List<tarea> tareas = [];
    Response response;
  ...
  @override
  Future<Either<MyError, List<Nota>>> getNotesByKeyword(
      String palabraClave, String idUsuario) async {
    List<Nota> notas = [];
    List<etiqueta> etiquetas = [];
    List<tarea> tareas = [];
    Response response;
    var body =
        jsonEncode({"palabraClave": palabraClave, "idUsuario": idUsuario});

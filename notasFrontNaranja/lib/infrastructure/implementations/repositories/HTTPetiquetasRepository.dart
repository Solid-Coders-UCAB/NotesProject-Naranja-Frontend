...
class HTTPetiquetasRepository extends HTTPrepository
    implements etiquetaRepository {
  @override
  Future<Either<MyError, String>> createEtiqueta(etiqueta etiqueta) async {
    var body = jsonEncode(
        {"nombreEtiqueta": etiqueta.nombre, "idUsuario": etiqueta.idUsuario});

    try {
      final Response response = await post(
          Uri.parse('https://$domain/etiqueta/create'),
          body: body,
          headers: {
  ...
  @override
  Future<Either<MyError, List<etiqueta>>> getAllEtiquetas(
  ...
  Future<Either<MyError, String>> updateEtiqueta(etiqueta etiqueta) async {
    ...
    try {
      final Response response = await put(
          Uri.parse('https://$domain/etiqueta/modificate'),
          body: body,
          headers: {
            "Accept": "application/json",
            "content-type": "application/json"
  ...
  Future<Either<MyError, String>> deleteEtiqueta(String idEtiqueta) async {
    ...
    if (r1.statusCode != 200) {
      return Left(MyError(key: AppError.NotFound, message: r1.body));
    }
  ...
  Future<Either<MyError, etiqueta>> getEtiquetaById(String idEtiqueta) async {
    ...
    try {
      final Response response = await post(
          Uri.parse('https://$domain/etiqueta/findById'),
          body: body,
          headers: {
            "Accept": "application/json",
            "content-type": "application/json"
  ...
  @override
  Future<Either<MyError, List<Nota>>> getNotesByEtiqueta(
      String idEtiqueta, String idUsuario) async {
    List<Nota> notas = [];
    List<etiqueta> etiquetas = [];
    List<tarea> tareas = [];

    Response response;
    var body = jsonEncode({"idEtiqueta": idEtiqueta, "idUsuario": idUsuario});
    ...
    if (response.statusCode == 200) {
      ...
      for (var jsonNote in jsonData) {
        ...
        Nota nota = Nota.create(
                id: jsonNote['id']['UUID'],
                titulo: jsonNote['titulo']['titulo'],
                contenido: jsonNote['cuerpo']['cuerpo'],
                n_edit_date:
                    DateTime.tryParse(jsonNote['fechaModificacion']['fecha']),
        ...
        var jsonAssignedGeolocalitation =
            jsonNote['geolocalizacion']['assigned'];
